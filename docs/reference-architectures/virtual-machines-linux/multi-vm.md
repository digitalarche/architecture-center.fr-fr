...
Cette architecture repose sur celle décrite dans l’article [Exécuter une machine virtuelle Linux dans Azure][single vm]. Les recommandations qui y sont fournies s’appliquent également à cette architecture.
...
Un déploiement pour cette architecture est disponible sur [GitHub][github-folder]. Il comprend un réseau virtuel, un groupe de sécurité réseau et trois machines virtuelles dans un groupe identique derrière un équilibreur de charge, comme décrit ci-dessous :
