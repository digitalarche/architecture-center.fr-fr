...
ms.locfileid: "53120048"
---
# <a name="run-a-linux-virtual-machine-on-azure"></a>Exécuter une machine virtuelle Linux dans Azure

Cet article présente un ensemble de pratiques éprouvées pour exécuter une machine virtuelle Linux dans Azure. Elle contient des recommandations pour le provisionnement de la machine virtuelle et des composants réseau et de stockage. [**Déployez cette solution**](#deploy-the-solution).
...
- **Réseau virtuel**. Chaque machine virtuelle Azure est déployée dans un réseau virtuel qui peut être segmenté en plusieurs sous-réseaux.
...
1. Créez une paire de clés SSH. Pour plus d’informations, consultez [Comment créer et utiliser une paire de clés publique et privée SSH pour les machines virtuelles Linux dans Azure](/azure/virtual-machines/linux/mac-create-ssh-keys).

2. Accédez au dossier `virtual-machines/single-vm/parameters/linux` pour le dépôt que vous avez téléchargé à l’étape des prérequis ci-dessus.

3. Ouvrez le fichier `single-vm-v2.json` et entrez votre nom d’utilisateur et votre clé publique SSH entre les guillemets, puis enregistrez le fichier.
...
4. Exécutez `azbb` pour déployer l’exemple de machine virtuelle, comme illustré ci-dessous.
